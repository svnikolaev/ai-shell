...
pub fn ask(question: &str, config: &Config) -> anyhow::Result<(String, String)> {
    ...
    anyhow::bail!("Все бэкенды недоступны или вернули ошибку.")
}
...
fn try_backend(
    ...
    let body = serde_json::json!({
        ...
        "temperature": 0,
        "max_tokens": 300,
        "response_format": {"type": "json_object"}  // некоторые модели игнорируют, но не мешает
    });
    ...
    let response = request.send_json(body).with_context(|| {
        format!(
    ...
    let content = json["choices"][0]["message"]["content"]
        .as_str()
        .ok_or_else(|| anyhow::anyhow!("Не удалось извлечь content из ответа"))?;
    ...
    let command = parsed["command"]
        .as_str()
        .ok_or_else(|| anyhow::anyhow!("Поле command не найдено или не строка"))?
        .to_string();
    let explanation = parsed["explanation"]
        .as_str()
        .ok_or_else(|| anyhow::anyhow!("Поле explanation не найдено или не строка"))?
        .to_string();
